  ...
  if (now - shtTimer >= SHT_INTERVAL) {
    ...
    if (ok && !isnan(t) && !isnan(h) && t > 0 && t < 60 && h > 0 && h <= 100) {
      ...
      if (thresholdReady) {
        controlSystem(t, h);
        String alert = "";
        if (t >= fan_temp        + 1.1f) alert += "อุณหภูมิสูงเกิน: " + String(t,1) + "C\n";
        if (t <= heater_off_temp - 1.1f) alert += "อุณหภูมิต่ำเกิน: " + String(t,1) + "C\n";
        if (h > hum_off + 1.1f)  alert += "ความชื้นสูงเกิน: " + String(h,1) + "%\n";
        if (h < hum_on  - 1.1f)  alert += "ความชื้นต่ำเกิน: " + String(h,1) + "%\n";
